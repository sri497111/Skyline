...
SNOWVERTEXSHADER = load_shader('./Shaders/snow.vert')
SNOWFRAGSHADER = load_shader('./Shaders/snow.frag')
...
class SnowShaderOverlay(QOpenGLWidget):
    def __init__(self, parent=None, pixmap=None):
        super().__init__(parent)
        self.pixmap = pixmap
        self.start_time = time.time()
    ...
    def initializeGL(self):
        ...
        self.program = glCreateProgram()
        glAttachShader(self.program, self.compile_shader(SNOWVERTEXSHADER, GL_VERTEX_SHADER))
        glAttachShader(self.program, self.compile_shader(SNOWFRAGSHADER, GL_FRAGMENT_SHADER))
        glLinkProgram(self.program)
